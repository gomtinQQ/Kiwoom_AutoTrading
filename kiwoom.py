...
class Kiwoom(QAxWidget):
	...
	def kiwoom_TR_OPT10080_주식분봉차트조회(self, strCode, tick=1, fix=1, size=240, nPrevNext=0, **kwargs):
		...
		res = self.kiwoom_SetInputValue("종목코드", strCode)
		res = self.kiwoom_SetInputValue("기준일자", "20190920")
	...
	def kosdaq_read(self):
		checkcount = 1

		with open('../data/kosdaq.csv', newline='', encoding='utf-8') as csvfile:
			kosdaq = csv.reader(csvfile, delimiter=',')
			for row in kosdaq:
				try:
					if(int(row[0]) == checkcount):
						checkcount += 1
						self.stockcode.append(row[1])
						self.stockname.append(row[2])
						self.capital.append(row[3])
						self.price.append(row[4])
				except:
					pass
		print("갯수 : " + str(len(self.stockcode))+"개")
...
if __name__ == '__main__':
	...
	# 분봉차트조회
	stockcode = hts.kosdaq_read()
	for sc in tqdm(stockcode[44+70:1364]):
		try:
			ohlcv = {'date': [], 'price': [], 'volume': []}
			data = []

			data = (hts.kiwoom_TR_OPT10080_주식분봉차트조회(sc, size=900*10, nPrevNext=0))
			for i in range(10):
				time.sleep(4.0)
				print(sc+"을 "+str(i)+ "번째 조회중 : "+ str(data['result'][-1]['time'])+ "부터 조회함")
				data.update(hts.kiwoom_TR_OPT10080_주식분봉차트조회(sc, size=900*10, nPrevNext=2))

			for i in range(0, len(data["result"])):
				ohlcv['date'].append(data["result"][i]["time"])
				ohlcv['price'].append(data["result"][i]["현재가"])
				ohlcv['volume'].append(data["result"][i]["volume"])

			df = pd.DataFrame(ohlcv, columns=['price', 'volume'], index=ohlcv['date'])
			con = sqlite3.connect("stock.db")
			df.to_sql(sc, con, if_exists='replace')
		except:
			print("ERROR")
			pass
	pass